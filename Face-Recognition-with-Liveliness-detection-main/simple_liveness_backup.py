...
class RobustLivenessDetector:
    def __init__(self):
        ...
        self.BRIGHTNESS_SPIKE_FACTOR = 1.8
    ...
    def detect_blink(self, landmarks):
        ...
        try:
            ...
            if len(self.blink_frames) >= 5:
                recent_ears = self.blink_frames[-5:]
                if (recent_ears[0] > 0.25 and min(recent_ears[1:4]) < 0.22 and recent_ears[-1] > 0.25):
                    ear_variance = np.var(recent_ears)
                    if ear_variance > 0.005:
                        self.blink_done = True
                        print("✅ NATURAL BLINK PATTERN DETECTED!")
        except Exception as e:
    ...
    def detect_head_movement(self, face_box, frame_width):
        ...
        if len(self.head_positions) >= 8:
            positions = self.head_positions[-8:]
            movement_deltas = np.diff(positions)
            movement_smoothness = np.std(movement_deltas)


            # Check for left turn
            if not self.head_left_done and movement_pct < -8:
                consistent_direction = sum(1 for d in movement_deltas if d < -0.3)
                if consistent_direction >= 3 and movement_smoothness < 8.0:
                    self.head_left_done = True
                    print("✅ NATURAL HEAD LEFT MOVEMENT DETECTED!")


            # Check for right turn
            if not self.head_right_done and movement_pct > 8:
                consistent_direction = sum(1 for d in movement_deltas if d > 0.3)
                if consistent_direction >= 3 and movement_smoothness < 8.0:
                    self.head_right_done = True
                    print("✅ NATURAL HEAD RIGHT MOVEMENT DETECTED!")
    ...
    def detect_mouth(self, landmarks):
        ...
        try:
            ...
            if len(self.mouth_values) >= 6:
                recent_mars = self.mouth_values[-6:]
                baseline = min(recent_mars[:3])
                peak = max(recent_mars[2:5])
                
                if (baseline < 0.35 and peak > 0.5 and peak - baseline > 0.2):
                    mar_variance = np.var(recent_mars)
                    if mar_variance > 0.01:
                        self.mouth_done = True
                        print("✅ NATURAL MOUTH OPENING PATTERN DETECTED!")
        except Exception as e:
